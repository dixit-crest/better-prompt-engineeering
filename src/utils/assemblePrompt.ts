...
export function assemblePrompt(prompt: FullPrompt): string {
  ...
  if (prompt.specStructure.enabled) {
    ...
    if (prompt.specStructure.preconditions.trim())
      specParts.push(`**Preconditions:** ${prompt.specStructure.preconditions}`)
    if (prompt.specStructure.inputs.trim())
      specParts.push(`**Inputs:** ${prompt.specStructure.inputs}`)
    if (prompt.specStructure.actions.trim())
      specParts.push(`**Actions:** ${prompt.specStructure.actions}`)
    if (prompt.specStructure.outputs.trim())
      specParts.push(`**Outputs:** ${prompt.specStructure.outputs}`)
    if (prompt.specStructure.postconditions.trim())
      specParts.push(`**Postconditions:** ${prompt.specStructure.postconditions}`)
  ...
  if (prompt.behaviorSections.enabled) {
    ...
    if (prompt.behaviorSections.happyPath.trim())
      behaviorParts.push(`**Happy Path:** ${prompt.behaviorSections.happyPath}`)
    if (prompt.behaviorSections.edgeCases.trim())
      behaviorParts.push(`**Edge Cases:** ${prompt.behaviorSections.edgeCases}`)
    if (prompt.behaviorSections.failureStates.trim())
      behaviorParts.push(`**Failure States:** ${prompt.behaviorSections.failureStates}`)
