...
function App() {
  ...
  const handleAutoFill = useCallback(async () => {
    ...
    if (suggestions) {
      ...
      if (suggestions.suggestedSuccessCriteria) {
        updateSection('successCriteria', { ...prompt.successCriteria, value: suggestions.suggestedSuccessCriteria });
      }
      if (suggestions.suggestedContext) {
        updateSection('context', { ...prompt.context, value: suggestions.suggestedContext });
      }
      ...
      if (suggestions.suggestedFunctionalRequirements) {
        updateSection('functionalRequirements', { ...prompt.functionalRequirements, value: suggestions.suggestedFunctionalRequirements.join('\n') });
      }
      if (suggestions.suggestedUserStories) {
        updateSection('userStories', { ...prompt.userStories, value: suggestions.suggestedUserStories.join('\n') });
      }
      if (suggestions.suggestedBusinessRules) {
        updateSection('businessRules', { ...prompt.businessRules, value: suggestions.suggestedBusinessRules.join('\n') });
      }
      if (suggestions.suggestedArchitecture) {
        updateSection('technicalArchitecture', { ...prompt.technicalArchitecture, value: suggestions.suggestedArchitecture });
      }
      if (suggestions.suggestedApiSpecs) {
        updateSection('apiSpecifications', { ...prompt.apiSpecifications, value: suggestions.suggestedApiSpecs });
      }
      if (suggestions.suggestedBehaviorSpecs) {
        updateSection('behaviorSpecifications', { ...prompt.behaviorSpecifications, value: suggestions.suggestedBehaviorSpecs });
      }
      if (suggestions.suggestedNonFunctionalRequirements) {
        updateSection('nonFunctionalRequirements', { ...prompt.nonFunctionalRequirements, value: suggestions.suggestedNonFunctionalRequirements.join('\n') });
      }
      if (suggestions.suggestedTesting) {
        updateSection('testingStrategy', { ...prompt.testingStrategy, value: suggestions.suggestedTesting.join('\n') });
      }
      if (suggestions.suggestedConstraints) {
        updateSection('constraints', { ...prompt.constraints, value: suggestions.suggestedConstraints.join('\n') });
      }
  ...
  }, [prompt.mainPrompt.value, autoFill, updateTechnologies, addSubtask, addSuccessGoal, updateSection, prompt, addDataExample]);
            ...
            <div className="p-8 max-w-5xl mx-auto space-y-8">
              <section className="space-y-4">
                <ModelConfigSection
                  ...
                  onTestConnection={async () => { await checkConnection(); }}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20">
                ...
                <SuccessCriteriaSection
                  ...
                  onChange={(val) => updateSection('successCriteria', { ...prompt.successCriteria, value: val })}
                />

                <ContextSection
                  context={prompt.context}
                  onChange={(val) => updateSection('context', { ...prompt.context, value: val })}
                />
                ...
                <FunctionalRequirementsSection
                  ...
                  onChange={(val) => updateSection('functionalRequirements', { ...prompt.functionalRequirements, value: val })}
                />

                <UserStoriesSection
                  userStories={prompt.userStories}
                  onChange={(val) => updateSection('userStories', { ...prompt.userStories, value: val })}
                />

                <BusinessRulesSection
                  rules={prompt.businessRules}
                  onChange={(val) => updateSection('businessRules', { ...prompt.businessRules, value: val })}
                />

                <TechnicalArchitectureSection
                  architecture={prompt.technicalArchitecture}
                  onChange={(val) => updateSection('technicalArchitecture', { ...prompt.technicalArchitecture, value: val })}
                />

                <ApiSpecificationsSection
                  specifications={prompt.apiSpecifications}
                  onChange={(val) => updateSection('apiSpecifications', { ...prompt.apiSpecifications, value: val })}
                />

                <BehaviorSpecificationsSection
                  specifications={prompt.behaviorSpecifications}
                  onChange={(val) => updateSection('behaviorSpecifications', { ...prompt.behaviorSpecifications, value: val })}
                />

                <NonFunctionalRequirementsSection
                  requirements={prompt.nonFunctionalRequirements}
                  onChange={(val) => updateSection('nonFunctionalRequirements', { ...prompt.nonFunctionalRequirements, value: val })}
                />

                <TestingStrategySection
                  strategy={prompt.testingStrategy}
                  onChange={(val) => updateSection('testingStrategy', { ...prompt.testingStrategy, value: val })}
                />

                <ConstraintsSection
                  constraints={prompt.constraints}
                  onChange={(val) => updateSection('constraints', { ...prompt.constraints, value: val })}
                />

                <SpecStructureSection
                  structure={prompt.specStructure}
                  onChange={(field, val) => updateSection('specStructure', { ...prompt.specStructure, [field]: val })}
                />

                <BehaviorSectionsSection
                  sections={prompt.behaviorSections}
                  onChange={(field, val) => updateSection('behaviorSections', { ...prompt.behaviorSections, [field]: val })}
                />
              </div>
            </div>
          </ScrollArea>
